...
class FormInstallationQuestions extends Form {
	...
	private function validateDatabase() {
		...
		try {
			...
			$this->getElement('dbName')->setValidationError('Settings table does not exist. Did you insert the setup/databases/schema.sql file?');
			return;
		}
	...
	private function validateDatabaseInitialData() {
		...
		if (count($settings) == 0) {
			$this->getElement('dbName')->setValidationError('There is nothing in the settings table. Did you insert the setup/databases/initialData.sql file?');
		}
	}
	...
	public function process() {
		...
		redirect('login.php', 'LPS installed.'); // We wrote the config file okay, redirect. 
	}
	...
	private function createAdministratorAccount() {
		...
		$sql = 'INSERT INTO users (username, password, registered, real_name, `group`) VALUES (:adminUsername, :adminPassword, now(), "Administrator", 1) ';
		$stmt = $this->db->prepare($sql);
